...
def test_create_timed_token():
    ...
    """
    # no duration
    tok1 = token.create_timed_token(sub="myuser", iss="xxx::xxxx", secret=SECRET, duration=None)
    assert jwt.decode(tok1, SECRET, algorithms=token.JWT_ALGO) == {
        "iss": "xxx::xxxx",
        "sub": "myuser",
    }

    # default duration
    tok2 = token.create_timed_token(sub="myuser", iss="xxx::xxxx", secret=SECRET, duration=600)
    assert jwt.decode(tok2, SECRET, algorithms=token.JWT_ALGO) == {
        "iss": "xxx::xxxx",
        "sub": "myuser",
        "exp": 1615570980,
    }
    ...
    with raises(TypeError):
        token.create_timed_token(sub="myuser", iss="xxx::xxxx", secret="")

    # missing sub
    with raises(TypeError):
        token.create_timed_token(sub="", iss="xxx::xxxx", secret=SECRET)
...
def test_validate_token():
    ...
    assert token.validate_token(tok1, iss="xxx:xxxx", secret=SECRET)["sub"] == sub

    # token we created, with expiration of 10 minutes
    tok2 = token.create_timed_token(sub, iss="xxx:xxxx", secret=SECRET, duration=600)
    payload2 = token.validate_token(tok2, iss="xxx:xxxx", secret=SECRET)
    ...
    with raises(jwt.exceptions.ExpiredSignatureError):
        token.validate_token(tok3, SECRET)

    _my_default = object()

    # correct token, incorrect secret
    secretx = SECRET[:-2]
    assert token.validate_token(tok2, secretx, _my_default) is _my_default

    # token is None
    assert token.validate_token(None, SECRET, _my_default) is _my_default
...
def patched_botoclient():
    ...
    """
    with patch.object(token.boto3, "client", autospec=True) as cli:
        cli.return_value.get_secret_value.return_value = {"SecretString": "asdfasdfasdf"}
        yield cli
...
def test_get_secret(patched_botoclient):
    ...
    Do we get a secret?
    """
    sec = token.get_secret("hello-world", "tester", "us-west-2")
...
def test_get_secret_error(patched_botoclient_bad):
    ...
    with raises(ClickException):
        token.get_secret("hello-world", "tester", "us-west-2")
