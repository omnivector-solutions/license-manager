from lm_cli.text_tools import copy_to_clipboard


def test_copy_to_clipboard__success(mocker):
    """
    Validate that the ``copy_to_clipboard`` function calls
    ``pyperclip.copy`` successfully and returns ``True``.
    """
...
def test_copy_to_clipboard__does_not_raise_exception_on_failure(mocker):
    """
    Validate that the ``copy_to_clipboard`` function catches exceptions
    from ``pyperclip.copy`` and returns ``False``.
    """
