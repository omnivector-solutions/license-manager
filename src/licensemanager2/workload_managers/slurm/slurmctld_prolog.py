...
def _check_feature_token_availablity(booking_request: dict) -> bool:
    ...
    for item in resp.json():
        product_feature = item["product_feature"].split(".")[0]
    ...
    if len(licenses) > 0:
        ...
        if _check_feature_token_availablity(booking_request):
            ...
            if _make_booking_request(booking_request):
                ...
                log.debug(f"Booking request unsuccessful.")
        else:
            log.debug(f"Not enough feature tokens for job to proceed.")
