...
from lm_backend.api.crud.generic_crud import CRUDGeneric
from lm_backend.database.models import LicenseServer
from lm_backend.api.api_schemas import LicenseServerCreateSchema, LicenseServerUpdateSchema
from lm_backend.database.session import get_session
from sqlalchemy.orm import Session

router = APIRouter()


license_server_crud = CRUDGeneric[LicenseServer, LicenseServerCreateSchema, LicenseServerUpdateSchema](model=LicenseServer)
...
@router.post("/", response_model=LicenseServer)
async def create_license_server(license_server: LicenseServerCreateSchema, session: Session = Depends(get_session)):
    return license_server_crud.create(session=session, obj_in=license_server)
