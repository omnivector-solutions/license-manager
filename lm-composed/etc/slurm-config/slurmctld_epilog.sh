#!/bin/bash
#
# Slurm EpilogSlurmctld script for license-manager
#
# This script is called by slurmctld when a job ends.
# It reads job context from Slurm environment variables and calls the
# license-manager epilog API endpoint.
#
...
#   LM_PROLOG_EPILOG_API_URL - Base URL for the prolog/epilog API (default: http://lm-agent-all:8081)
#

set -e

# Configuration
LM_PROLOG_EPILOG_API_URL="${LM_PROLOG_EPILOG_API_URL:-http://lm-agent-all:8081}"

# Log function
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] EPILOG: $*" >&2
}
...
# Main function
main() {
    log "Starting epilog for job ${SLURM_JOB_ID:-unknown}"
...
EOF
    ...
    log "Calling epilog API at ${LM_PROLOG_EPILOG_API_URL}/epilog"
    log "Payload: $payload"
    ...
    response=$(curl -s -w "\n%{http_code}" \
        -X POST "${LM_PROLOG_EPILOG_API_URL}/epilog" \
    ...
    if [[ "$http_code" -ge 200 && "$http_code" -lt 300 ]]; then
        log "Epilog completed successfully for job ${SLURM_JOB_ID}"
        exit 0
    else
        log "ERROR: Epilog failed for job ${SLURM_JOB_ID} (HTTP $http_code)"
        exit 1
    fi
}
